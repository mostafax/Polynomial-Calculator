...
def display(eq):
    ...
    for i in range(0, len(eq)):
        ...
        if (final[i][0] != 0):
            ...
            if (str(final[i][0]) > str(0) and i != 0):
                string_final += "+"
...
def calc_diff(MonoElements, MonoElements2):
    ...
    for x in range(0, MonoElements2_size):
        ...
        for z in range(0, MonoElements_size):
            ...
            if MonoElements[z][1] == MonoElements2[x][1] and MonoElements[z][2] == MonoElements2[x][2] and \
                ...
                result.append(
                    ((int(MonoElements[z][0]) - int(MonoElements2[x][0])), MonoElements[z][1], MonoElements[z][2],
    ...
    length = len(result)
    for x in range(0,
                   length):  # from x because we(-)to x and to length-x because the length will decrease by the the elements we (-) it
